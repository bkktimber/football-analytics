...
col_names = [
    ...
    'attendance', 'venue', 'referee', 'match_report_url', 'tmp1'
]
...
# %%
t0 = time.time()
target_page = 'https://fbref.com/en/comps/11/schedule/Serie-A-Scores-and-Fixtures'
headers = {'User-Agent': 'Mozilla/5.0'}
response = requests.get(target_page, headers=headers)
t1 = time.time()
print(f'take {t1-t0:.2f} s')
# %%
check_path = 'fbref/figc/20192020/matches/*/*'
matches = glob(check_path)
if matches:
    last_match = max([int(p.split('_')[-1]) for p in matches])
else:
    last_match = 0
...
table = soup.find('div', {
    ...
    'id': 'div_sched_ks_3260_1'
})
table = table.find('table', {'id': 'sched_ks_3260_1'})
...
df.to_csv(
    '/Users/Mai/Projects/ideas/open-data/fbref/figc/20192020/fixtures.csv')
t1 = time.time()
print(f'take {t1-t0:.2f} s')

# %%
save_file = '/Users/Mai/Projects/ideas/open-data/fbref/figc/20192020/matches/'
...
# %%
print(f'Downloaded all 380 matches from {target_page}')
