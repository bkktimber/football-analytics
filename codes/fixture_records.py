...
nufc = 'Newcastle Utd'
df = pd.read_csv('/Users/Mai/Projects/football-analytics/data/epl_fixture_20202021.csv')
...
season_order = dict([(v, idx) for idx, (k,v) in enumerate(season_mapping.items())])
# %%
team_names = sorted(set(nufc_fixtures.home_team.tolist()))
team_crests = sorted(set(os.listdir(crest_dir)))
team_crest_mapping = dict([(n, os.path.join(crest_dir, p)) for n, p in zip(team_names, team_crests)])
# %%
opponentes = []
...
# %%
selected_cols = ['match_id', 'gameweek', 'home_team', 'score_home', 'score_away', 'away_team']
results = []
...
ax_fixture.imshow(20 - x, cmap="Purples", alpha=0.85,
                 origin="lower", vmin=0)
...
for idx, row in plot.iterrows():
    ...
    if lst[idx][0] >= 0.5:
        ax_result.scatter(x=idx, y=avg_score, c='#2a8635',
        s=160, )
    else:
        ax_result.scatter(x=idx, y=avg_score, c='#2a8635',
        s=160, alpha=0.3)
...
ax_result.set_ylabel(f'Average Points (last 10 seasons)',
                    fontweight='bold')
...
plt.suptitle(f'{nufc} Campaign Difficulty\n Expected points {sum(tmp):.2f} points',
            fontweight='bold', color='#555555', size='x-large')
