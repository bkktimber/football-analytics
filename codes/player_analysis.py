...
player_def_col = ['rank', 'player_name', 'nationality','position',
'squad', 'age', 'year_of_birth', '90s',
 'tackles', 'tackle_won',
 'tackles_in_def_third',
 'tackles_in_mid_third',
 'tackles_in_atk_third',
 'tackles_won_dribble', 'tackle_attempts_dribble', 'tackle_dribble_percentage',
 'dribble_passed',
 'pressure', 'pressure_success', 'pressure_percentage',
 'pressure_in_def_third',
 'pressure_in_mid_third',
 'pressure_in_atk_third',
 'blocks',
 'shot_blocked', 'shot_saved',
 'pass_blocks',
 'interception',
 'clearance',
 'error',
...
player_pos_col = ['rank', 'player_name', 'nationality','position',
'squad', 'age', 'year_of_birth', '90s',
 'touchs',
 'touchs_in_def_pen',
 'touchs_in_def_third',
...
df_all_player_def = pd.read_csv('/Users/Mai/Projects/ideas/open-data/fbref/individuals/epl/individual-defense-20192020.csv', header=1, names=player_def_col)
df_all_player_pos = pd.read_csv('/Users/Mai/Projects/ideas/open-data/fbref/individuals/epl/individual-posession-20192020.csv', header=1, names=player_pos_col)
...
col_list = ['xa', 'pass_complete_percentage',
            'progressive_pass', 'key_passes', 
            'tackle_won', 'pressure', 'pressure_success',
            'blocks', 'interception']
p_col = 'player_name_pass'

_df = df_all_player_pass.join(df_all_player_def, lsuffix='_pass', rsuffix='_def')
for col in col_list:
    df = _df.loc[bool_filter, [p_col, col]].sort_values(by=col, ascending=True).reset_index(drop=True, inplace=False)
    num_player = df.shape[0]
    df.loc[:, p_col] = df.loc[:, p_col].str.split('\\').str[0]
    df['percentile'] = df.index * (100/num_player)
    epl_rf.append((df.loc[(df.loc[:, p_col] == rf), 'percentile']).tolist()[0]/100)
...
for col in col_list:
    ...
    df.loc[:, p_col] = df.loc[:, p_col].str.split('\\').str[0]
    df['percentile'] = df.index * (100/num_player)
    epl_ih.append((df.loc[(df.loc[:, p_col] == ih), 'percentile']).tolist()[0]/100)
...
for col in col_list:
    ...
    df.loc[:, p_col] = df.loc[:, p_col].str.split('\\').str[0]
    df['percentile'] = df.index * (100/num_player)
    epl_js.append((df.loc[(df.loc[:, p_col] == js), 'percentile']).tolist()[0]/100)
...
epl_ma = []
epl_asm = []
col_list = ['xa', 'assists', 'pass_complete_percentage', 'progressive_pass', 
            'pass_to_final_third', 'pass_to_penalty_area', 'key_passes',
            'shot_on_target_percentage', 'xg', 'goals',]
p_col = 'player_name_pass'

_df = df_all_player_pass.join(df_all_player_shot, lsuffix='_pass', rsuffix='_shot')
...
for col in col_list:
    ...
    df.loc[:, p_col] = df.loc[:, p_col].str.split('\\').str[0]
    df['percentile'] = df.index * (100/num_player)
    epl_ma.append((df.loc[(df.loc[:, p_col] == ma), 'percentile']).tolist()[0]/100)
...
for col in col_list:
    ...
    df.loc[:, p_col] = df.loc[:, p_col].str.split('\\').str[0]
    df['percentile'] = df.index * (100/num_player)
    epl_asm.append((df.loc[(df.loc[:, p_col] == asm), 'percentile']).tolist()[0]/100)
# %%
N = len(col_list)
data = [0]
